...
### parameter
1. **RandomForest** is the predicted solar irradiance [W/m^2] by using RandomForest model that the proposed model of the product.
2. **Iclr** is the clear-sky solar irradiance [W/m^2] (the irradiance in clear-sky condition)
...
def eva_table(df, name_model, base = 'I',):
    ...
    for i in range(len(name_model)):
        ...
        data.append({'Overall MAE [W/m^2]':MAE, 'Overall RMSE [W/m^2]':RMSE, 'MAE at noon [W/m^2]':noon_MAE, 'RMSE at noon [W/m^2]':noon_RMSE,
                    'MAE at non-noon [W/m^2]':non_noon_MAE, 'RMSE at non-noon [W/m^2]':non_noon_RMSE})
...
st.write("""
The model error table shown the error of the model in **Mean Absolute Error (MAE)** [W/m^2] and **Root Mean Square Error (RMSE)** [W/m^2] with 2 conditions. There are the error at nooon which mean during in 10.00 - 14.00 and the error at non-noon which mean during in before 10.00 and after 14.00
...
st.write("""
The hourly MAE [W/m^2] in each model.
""")
